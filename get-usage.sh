...
MERGE_FILES=true
ORG_HISTORY_FILE="$HOME/.circleci-usage-orgs.json"
...
# Save or update an org entry in ~/.circleci-usage-orgs.json
# Called automatically after a successful export so the org appears in the picker next time.
save_org_to_history() {
    local org_id="$1"
    local org_name="$2"
    local today
    today=$(date +%Y-%m-%d)
    python3 - "$ORG_HISTORY_FILE" "$org_id" "$org_name" "$today" << 'PYEOF'
import json, sys, os
path, org_id, org_name, today = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
try:
    with open(path) as f:
        orgs = json.load(f)
except Exception:
...
for org in orgs:
    if org.get('id') == org_id:
        ...
        break
if not found:
    orgs.append({'id': org_id, 'name': org_name, 'last_used': today})
orgs.sort(key=lambda x: x.get('last_used', ''), reverse=True)
with open(path, 'w') as f:
    json.dump(orgs, f, indent=2)
PYEOF
}

# Show a numbered menu of recently used orgs (sourced from ~/.circleci-usage-orgs.json).
# If no history exists yet, falls straight through to a plain text prompt.
# Echoes the selected org_id to stdout; all display goes to stderr.
prompt_for_org_id() {
    local count=0
    if [ -f "$ORG_HISTORY_FILE" ]; then
        count=$(python3 -c "import json; print(len(json.load(open('$ORG_HISTORY_FILE'))))" 2>/dev/null || echo "0")
    fi

    if [ "$count" -gt 0 ]; then
        echo -e "${YELLOW}Recent Organizations:${NC}" >&2
        echo "" >&2

        python3 - "$ORG_HISTORY_FILE" << 'PYEOF' >&2
import json, sys
with open(sys.argv[1]) as f:
    data = json.load(f)
for i, org in enumerate(data, 1):
    name = org.get('name', 'unknown')[:22].ljust(22)
    oid  = org.get('id', '')
    last = org.get('last_used', '')
    print(f"  [{i}] {name}  {oid}  (last used: {last})")
...
PYEOF
        ...
        if [[ "$choice" =~ ^[0-9]+$ ]] && [ "$choice" -ge 1 ] && [ "$choice" -le "$count" ]; then
            python3 - "$ORG_HISTORY_FILE" "$choice" << 'PYEOF'
import json, sys
with open(sys.argv[1]) as f:
    data = json.load(f)
...
PYEOF
            return 0
        elif [[ "$(echo "$choice" | tr '[:upper:]' '[:lower:]')" == "n" ]] || [ -z "$choice" ]; then
            : # fall through to manual entry below
        else
            echo -e "${RED}✗ Invalid selection — please enter a number or N${NC}" >&2
            return 1
        fi
    fi

    # No history yet, or user chose N — prompt for a new ID
    ...
    while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
        ...
        if [ "$STATE" = "finished" ] || [ "$STATE" = "completed" ]; then
            ...
            if [ "$MERGE_FILES" = true ]; then
                    ...
                    if [ -f "$MERGED_FILE" ]; then
                        ...
                        if [ -f "$MERGED_FILE" ]; then
                            ...
                            if [ -n "$RAW_ORG_NAME" ]; then
                                # Save this org to local history for the picker menu
                                save_org_to_history "$org_id" "$RAW_ORG_NAME"
                        ...
                        echo -e "${BLUE}   https://amarirules.github.io/circleci-usage-visualizer/${NC}"
                        echo -e ""
                ...
                if [ -n "$FIRST_CSV" ]; then
                    # Try to save org to history using name from CSV (no-merge path)
                    NO_MERGE_ORG_NAME=$(awk -F',' 'NR==2{gsub(/"/, "", $2); print $2}' "$FIRST_CSV" 2>/dev/null)
                    if [ -n "$NO_MERGE_ORG_NAME" ]; then
                        save_org_to_history "$org_id" "$NO_MERGE_ORG_NAME"
                    fi
